function __print_scandium_functions_help() {
cat <<EOF
Additional ScandiumOS functions:
...
- githubremote:    Add git remote for scandiumProject Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the scandium model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch scandium_$target-$variant
        fi
    fi
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Scandium-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.scandium.device | grep -q "$SCANDIUM_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $SCANDIUM_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/Scandium-OS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.scandium.device | grep -q "$SCANDIUM_BUILD");
    then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                scandium|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function dopush()
{
    ...
    if (adb shell getprop ro.scandium.device | grep -q "$SCANDIUM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SCANDIUM_FIXUP_COMMON_OUT ]; then
